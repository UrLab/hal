...
class Client:
    ...
    def __init__(self, url, key=None, secret=None, timeout=5, context=None):
        ...
        url_split = _parse_url(url)
        scheme = "https" if url_split['secure'] else "http"
        self._url = "{}://{}:{}{}".format(scheme,
                                          url_split['host'],
                                          url_split['port'],
                                          url_split['path'])
        self._headers = {
    ...
    def publish(self, topic, *args, **kwargs):
        ...
        url = "{0}?{1}".format(self._url, parse.urlencode(params))

        ## now issue the HTTP/POST
        ##
        print(url, body, self._headers)
        response = yield from aiohttp.post(url, data=body, headers=self._headers)
